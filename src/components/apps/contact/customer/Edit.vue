    ...
    <div class="header col-md-12">
      <div class="header-inner row align-items-center">
        ...
        <div class="col-md-8 right">
          <div class="action" v-if="customer !== null">
              ...
              <div class="col-md-3">
                <div class="form-group row">
                  <div class="col-md-12">
                    ...
                    <select class="custom-select" v-model="form.gender" disabled>
              ...
              <div class="col-md-3">
                <div class="form-group row">
                  <div class="col-md-12">
                    ...
                    <input type="text" class="form-control" ref="" :value="getDate(this.customer.birthdate)" disabled>
                  </div>
                </div>
              </div>
              <div class="col-md-12">
...
export default {
  ...
  methods: {
    async update (option = {}) {
      ...
      let res = await this.$store.dispatch('updateCustomer', this.data)
      ...
      if(res.success) {
        ...
        sidebarModule.update(this.sidebar, 1, [{
          ...
          "router": {
            ...
            "query": {
              ven_id: this.customer.customer_id,
              ven_name: this.customer.customer_name
            }
          },
          "activeClass": 'active'
        }], {
    ...
    async del () {
      ...
      let res = await this.$store.dispatch('deleteCustomer', data)
      ...
      if(res.success) {
        // show notify
        this.notify(this.$vs, {
          text: this.$t('message.hasbeendeleted', { msg: this.customer.customer_name }),
          color:'danger',
          icon: 'delete'
        })
        // close modal
  ...
  created: async function () {
    ...
    let res = await this.$store.dispatch('getCustomer', {
      customer_id: this.$route.query.cus_id
    })
    if(!res.success) {
      return this.$router.push({ name: "ContactCustomerAll"})
    }
     ...
     */
    // set form
    this.form.name = this.customer.customer_name
    this.form.gender = this.customer.gender
    this.form.birthdate = new Date(this.customer.birthdate)
    this.form.address = this.customer.address
    for(let i in this.customer.contacts) {
      let item = this.customer.contacts[i]
  ...
  beforeMount: function () {
    ...
    sidebarModule.init(this.getObject(require('../sidebar.json')))
    sidebarModule.update(this.sidebar, 1, [{
      "name": this.$route.query.cus_name,
      "router": {
        "name": "ContactCustomerEdit",
        "query": {
          cus_id: this.$route.query.cus_id,
          cus_name: this.$route.query.cus_name
        }
      },
      "activeClass": 'active'
    }], {
  ...
  destroyed: function() {
    this.leavePage(this.$Progress, ['clearCustomer'])
  }
}
</script>
