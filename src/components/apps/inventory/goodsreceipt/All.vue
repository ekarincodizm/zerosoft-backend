  ...
  <div id="content">
    <div class="header col-md-12">
      <div class="header-inner row align-items-center">
        <div class="col left">
          <div class="title">
              {{ $t('sidebar.goodsreceipt') }} > {{ $t('sidebar.all') }}
          </div>
        </div>
        <div class="col right">
          <select class="form-control form-control-sm" style="float: right;width: 120px;margin-left: 12px;" v-model="status">
            ...
            <option value="padding">{{ $t('message.paddingorder') }}</option>
            <option value="done">{{ $t('message.receivedorder') }}</option>
          </select>
            ...
            <td>
              <div class="dropdown">
                <button
                  ...
                  <span class="name text-warning">{{ $t('message.paddingorder') }}</span>
                </button>
                <button
                  class="btn no-bg"
                  type="button"
                  ...
                  <span class="name text-success">{{ $t('message.receivedorder') }}</span>
                </button>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton1">
                  <router-link class="dropdown-item" :to="{name: 'GoodsReceiptEdit', query: {
                    'gr_id': item.gr_id,
                    'gr_code': item.gr_code,
                    'modal': 'received-orders'
                    }}">
                    <i class="far fa-check-circle text-success"></i>
                    <span class="text-success">{{ $t('message.receivedorder') }}</span>
                  </router-link>
                </div>
              </div>
            </td>
...
export default {
  ...
  watch: {
    ...
    ready: function () {
      ...
      $.fn.dataTable.ext.search.push(
        ...
        {
          let warehouse_id = that.goodsreceipts[dataIndex].stockin.warehouse_id
          let status = that.goodsreceipts[dataIndex].status
  ...
  created: async function () {
    ...
    // get goodsreceipts
    await this.$store.dispatch('getGoodsreceipts')
    // set ready
    this.ready = true
  },
  ...
  destroyed: function() {
    // reset search
    $.fn.dataTable.ext.search = []
    this.leavePage(this.$Progress, ['clearGoodsreceipts'])
  }
}
</script>
