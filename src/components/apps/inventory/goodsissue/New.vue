      ...
      <template v-if="ready">
        <section>
          <div class="col-md-12 no-padding">
            <section-header :title="$t('message.customer')+'/'+$t('message.partner')" :subtitle="$t('message.refgicode')+' ' + nextGiCode"></section-header>
            <div class="row">
              <div class="col-md-12">
...
export default {
  ...
  methods: {
    ...
    async clearForm () {
      ...
      // get next grcode
      await this.$store.dispatch('getNextGiCode', {
    ...
    async save (option = {}) {
      ...
      // save data
      const res = await this.$store.dispatch('insertGoodsissue', data)

      console.log(res)

      if(res.success) {
        let gi_code = res.data.goodsissue.gi_code
        // show notify
        this.notify(this.$vs, {
          text: this.$t('message.hasbeencreated', { msg: gi_code }),
          color:'success',
          icon: 'cloud'
        })
  ...
  destroyed: function() { // leave this page
    this.leavePage(this.$Progress, ['clearCustomers', 'clearCustomer'])
  }
}
</script>
